...
// (EUR), and dividend-per-share estimates for the current year and the next two
// (T, T+1, T+2), EUR only. Batched via aliased queries.
...
const F =
  ...
  "estimateTransactions(first:1){ estimates { year quarter dividend } }";
...
async function fetchChunk(chunk, out) {
  ...
  chunk.forEach((isin, i) => {
    ...
    // dividend-per-share estimates for T, T+1, T+2 (full-year rows), EUR only
    let divEstimates = null;
    if ((inst.reportingCurrency || "EUR") === "EUR") {
      const rows = inst.estimateTransactions?.[0]?.estimates || [];
      const m = {};
      for (const e of rows) {
        if (e.quarter === 0 && DIV_YEARS.includes(e.year) && e.dividend != null) m[e.year] = e.dividend;
      }
      if (Object.keys(m).length) divEstimates = m;
    }

    if (!rec && !divEstimates) return;
    out[isin] = {
      rec,
